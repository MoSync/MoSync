...
#if defined(S_IXGRP) && !defined(__SYMBIAN)
...
{
    SafeStringValue(fname);
    //RVCT error #20: if (eaccess(StringValueCStr(fname), R_OK) < 0) return Qfalse;
    return Qtrue;
}
...
{
    SafeStringValue(fname);
    //RVCT error #20: if (access(StringValueCStr(fname), R_OK) < 0) return Qfalse;
    return Qtrue;
}
...
{
    SafeStringValue(fname);
    //RVCT error #20: if (eaccess(StringValueCStr(fname), W_OK) < 0) return Qfalse;
    return Qtrue;
}
...
{
    SafeStringValue(fname);
    //RVCT error #20: if (access(StringValueCStr(fname), W_OK) < 0) return Qfalse;
    return Qtrue;
}
...
{
    SafeStringValue(fname);
    //RVCT error #20: if (eaccess(StringValueCStr(fname), X_OK) < 0) return Qfalse;
    return Qtrue;
}
...
{
    SafeStringValue(fname);
    //RVCT error #20: if (access(StringValueCStr(fname), X_OK) < 0) return Qfalse;
    return Qtrue;
}
    ...
    void *arg;
{
    ...
    //Not implemented in SOS STDLIB: if (utime(path, utp) < 0)
	rb_sys_fail(path);
}
...
#  endif
/*
//Not implemented in SOS STDLIB C: 
    ...
    const char * const *ext;
{
	...
	*filep = fname;
	...
	for (j=0; ext[j]; j++) {
	    ...
	    OBJ_FREEZE(fname);
	    found = 0;//dln_find_file(StringValueCStr(fname), path);
