...
//TODO: ensure right operation on SOS.
#  define READ_DATA_PENDING(fp) (!feof(fp))
#  define READ_DATA_BUFFERED(fp) 0
#  define READ_DATA_PENDING_PTR(fp) ((char *)(getc(fp)))
#  define READ_DATA_PENDING_COUNT(fp) 0
    ...
    VALUE io, str;
{
	...
	str = rb_obj_as_string(str);
		...
		if(*RSTRING(str)->ptr != '\n')
			PrintConsole(RSTRING(str)->ptr);
		//maWriteLog(RSTRING(str)->ptr, RSTRING(str)->len);
		//maWriteLog("\n", 1);
		return LONG2FIX(RSTRING(str)->len);

		printf("TYPE");
    if (TYPE(io) != T_FILE) {
	/* port is not IO, call write method for it. */
			printf("rb_funcall, id_write\n");
...
next_argv()
{
	...
	if (RARRAY(rb_argv)->len > 0) {
		...
		if (ruby_inplace_mode) {
			...
			//TODO: fchown(fileno(fw), st.st_uid, st.st_gid);
		    }
#endif
