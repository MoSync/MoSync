...
#define RUBY_RELATIVE(path) (path)
    ...
    if (rb_safe_level() == 0) {
	ruby_incpush((void*)getenv("RUBYLIB"));
    }
/*
    ...
    char **argv;
{
    ...
    for (argc--,argv++; argc > 0; argc--,argv++) {
			MLOG("proc_option %i: \"%s\"\n", argc, argv[0]);
	if (argv[0][0] != '-' || !argv[0][1]) break;

	s = argv[0]+1;
      reswitch:
			MLOG("proc_option switch: %c\n", *s);
	switch (*s) {
	  case 'a':
	    do_split = Qtrue;
	    s++;
	  ...
	  default:
		...
		printf(format, origargv[0], (int)(unsigned char)*s);
	    }
	    exit(2);
  ...
  switch_end:
    ...
    if (rb_safe_level() == 0 && (s = (char*)getenv("RUBYOPT"))) {
	while (ISSPACE(*s)) s++;
	if (*s == 'T' || (*s == '-' && *(s+1) == 'T')) {
	    int numlen;
	    int v = 1;
	...
	if (argc == 0) {	/* no more args */
		...
		char *path = (char*)getenv("RUBYPATH");
		...
		if (!script) {
		    script = dln_find_file(argv[0], (char*)getenv(PATH_ENV));
		}*/
	...
	if (c == INT2FIX('#')) {
		...
		if ((p = strstr(RSTRING(line)->ptr, "ruby")) == 0) {
		    ...
		    *p++ = '\0';
		    ...
		    argv[0] = path;
		    //Not implemented in SOS STDLIB C: execv(path, argv);
