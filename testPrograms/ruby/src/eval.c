...
extern VALUE ruby_errinfo;// = Qnil;
...
ruby_init()
{
    ...
    PUSH_TAG(PROT_NONE);
		if ((state = EXEC_TAG()) == 0) {
			...
			MLOG("rb_node_newnode\n");
			top_cref = rb_node_newnode(NODE_CREF,rb_cObject,0,0);
			ruby_cref = top_cref;
			MLOG("rb_define_global_const\n");
    ...
    NODE *node;
{
    NODE *beg_tree = ruby_eval_tree_begin;
		...
		MLOG("eval_node: beg_tree 0x%08x self 0x%08x node 0x%08x\n", beg_tree, self, node);
...
ruby_finalize_1()
{
    //Not implemented in SOS STDLIB C: signal(SIGINT, SIG_DFL);
    ruby_errinfo = 0;
...
ruby_exec_internal()
{
	...
	//MLOG("EXEC_TAG\n");
    if ((state = EXEC_TAG()) == 0) {
	MLOG("ruby_exec_internal eval_node\n");
...
{{
  ...
  again:
	...
	n = 0;//use ioctl() from estlib.h instead of select(max+1, &readfds, &writefds, &exceptfds, delay_ptr);
	if (n < 0) {
	    int e = errno;
    ...
    struct timeval time;
{
	...
	for (;;) {
	    rb_thread_critical = Qtrue;
	    TRAP_BEG;
	    n = 0; //TODO: use ioctl() instead of select(0, 0, 0, 0, &time);
	    rb_thread_critical = thr_critical;
	    TRAP_END;
	    if (n == 0) return;
	    if (n < 0) {
    ...
    struct timeval *timeout;
{
	...
	for (;;) {
	    TRAP_BEG;
	    n = 0; //TODO: use ioctl() instead of select(max, read, write, except, tvp);
	    TRAP_END;
	    if (n < 0) {
		switch (errno) {
		  case EINTR:
    ...
    int sig;
{
    ...
    //Not implemented in SOS STDLIB C: signal(sig, catch_timer);
#endif
    /* cause EINTR */
}
...
#else
	//Not implemented in SOS STDLIB C: signal(SIGVTALRM, catch_timer);
#endif
