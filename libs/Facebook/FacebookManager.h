...
/**
   * \brief Class that manages the connections requests, Facebook objects
   * and publishing.
   */
...
public:
	...
	* constructor
	* @param appId - the application's id
	* @param permissions - the permissions that the application wants to
	* request.
	* @param appSecret - the application's secret.
	*/
	FacebookManager(const MAUtil::String &appId,
			const MAUtil::Set<MAUtil::String> &permissions);
	...
	*/
	...
	/**
	* Sets the ObjectRequestListener. The listener will be notified when a
	* Facebook object request is completed, or if the request has failed.
	*/
	void setObjectRequestListener(ObjectRequestListener *listener);

	/**
	* Requests a Facebook object.
	* The "Type" template parameter is the type of the object that is
	* requested.
	...
	* requested.
	* @param id - the id of the requested object.
	* @param fields - specifies the fieds to be requested from Facebook,
	* for the specified object. For example if we ask an Album, and
	* add to the "fields" Set "id" and "name", we will request from Facebook
	* only the id and name fields contained in the the Album object.
	*/
	template<class Type>
	void requestObject(const MAUtil::String &id,
	...
	/**
	* Sets a ConnectionsManagerListener, that will be informed when a
	* connection request is completed, or it had failed.
	*/
	...
	*		"permissions", "photos", "picture", "albums", "videos",
	*		"videos/uploaded", "events", "groups", "checkins", "notes",
	*		"comments", "noreply", "maybe", "attending", "declined", "members",
	*		"accounts", "activity",	"friendlists", "interests", "links",
	*		"posts", "statuses", "television"
	* @param id - the id of the object for which the connection was requested.
	*/
	void requestConnection(const MAUtil::String &connType,
	...
	*		"posts", "statuses", "television"
	* @param fields - specifies the fieds to be requested from Facebook,
	* for an object.For example if we request the "albums" connection, and
	* add to the "fields" Set "id" and "name", we will request from Facebook
	* Album objects with only the id and name.
	* @param id - the id of the object for which the connection was requested.
	*/
	...
	*/
	...
	/**
	* Function called when all the JSON data retrieved from server must be
	* parsed and trasformed into Facebook objects.
	...
	*/
	...
	/**
	* Sets the PublishingListener, that will be informed about the result of
	* the publish request.
	*/
	...
	/**
	* Publishes a Like
	* @param id - the id of the object to be liked ( a post, message,
	* comment ect).
	*/
	...
	/**
	 * Publishes a Unlike
	 * @param id - the id of the object to be disliked ( a post, message,
	 * comment ect).
	 */
	...
	* @param response - the response to the event. Can be: attending, maybe,
	* declined
	*/
	...
	/**
	* Adds a Note.
	* @param OBJECT_ID - the id of the User for which the Note object is
	* created.
	* @param subject - the subject of the Note.
	* @param message - the message of the Note.
	*/
	void addNote(const MAUtil::String &OBJECT_ID,
			const MAUtil::String &subject,
			const MAUtil::String &message);
	/**
	* Adds a Album object.
	* @param OBJECT_ID - the id of the User for which the Album object
	* is created.
	...
	* @param privacyType - string containing 'OPEN' (default), 'CLOSED',
	* or 'SECRET'
	*/
	void addEvent(const MAUtil::String &PROFILE_ID,
			const MAUtil::String &eventName,
			const UnixTimeStamp &eventStart_time,
	...
	*/
	void addPhoto(const MAUtil::String &OBJECT_ID, const byte *pixels,
			int pixelsArraySize, const MAUtil::String &message="");
	...
	* @param videoDescpription - the video description, that will be posted
	...
	* @param PROFILE_ID - the id of the User for which the Checkin object is
	* created.
	...
	*/
	void addCheckin(const MAUtil::String &PROFILE_ID,
			const MAUtil::String &placeId, const Coordinate &coord,
	...
	* @param message - the message to be posted
	* @param id - the id of the user on which wall the status message will be
	* displayed.
	*/
...
private:
	...
	/**
	* initializes the FacebookManager: creates the Facebook object, and
	* creates the ConnectionsManager, the FacebookObjectManager and the
	* FacebookPublisher2, and registers the FacebookManager to them.
	* @param id - the application id.
	* @param permissions - the permissions to request from Facebook.
	* @param appSecret - the appliction's secret.
	*/
	void initialize(const MAUtil::String &appId,
			const MAUtil::Set<MAUtil::String> &permissions);

private:

	/**
	*  FacebookObjectManager - handles the request of Facebook objects and
	*  the retrieving of the server's response.
	*/
	FacebookObjectManager			*mObjectManager;

	/**
	* ConnectionsManager - handles the request of connections and the
	* retrieving of the server's response.
	*/
	ConnectionsManager				*mConnectionsManager;

	/**
	* FacebookPublisher2 - handles the publish requests and the retrieving
	* of the server's response.
	*/
	FacebookPublisher2				*mPublishingManager;

	/**
	* Facebook - low level class, for comunicating with the server.
	* It creates the requests and sends them. Also retrieves the JSON data,
	* and informs the ConnectionsManager, sending it the JSON data.
	* ConnectionsManager parses the JSON data, and transforms it into
	* Facebook objects. Those will be send to the FacebookManager.
	*/
	Facebook						*mFacebook;
};
