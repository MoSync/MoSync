...
/**
 * @file FacebookObjectManager.h
 * @author Gabriela Rata
 */

#ifndef FACEBOOK_OBJECT_MANAGER_H_
#define FACEBOOK_OBJECT_MANAGER_H_
...
/**
 * \brief Base class for RequestHelper.
 */
...
/**
 * \brief Helper class for FacebookObjectManager
 * It is implemented to send a request for a given object type and then be
 * deleted.
 * The template parameter "ObjectType" - the type of the object we are
 * requesting.
 */
...
public:
	 ...
	 * @param listener - listener that will be notified when the request is
	 * completed or the request failed.
	 */
	RequestHelper(Facebook *facebook, ObjectRequestListener *listener):
		mFacebook(facebook), mListener(listener)
	{
	 ...
	 * Function that request a Facebook object
	 * @param id - the id of the object that we want to retrieve from Facebook
	 * @param fields - the properties of the object that we want to retrieve
	 * from server. For example we can ask only for name, or name and message.
	 */
	virtual void requestFacebookObject(const MAUtil::String &id,
...
/**
 * \brief Class that manages requesting and retrieving Facebook objects.
...
public:
	 ...
	 * @param id - the id of the object that we want to retrieve from Facebook
	 * The template parameter "Type" - is the type of the object we are
	 * requesting.
	 ...
	 * @param listener - the listener of the ObjectRequestListener. It is
	 * informed by the result of the request.
	 * On success the facebookObjectReceived function is called and the object
	 * is send as a parameter.
	 * On failure the error code and the id of the object, for which the request
...
private:
	...
	/**
	 * mListener - pointer to a ObjectRequestListener. It will be notified by
	 * the result of the request.
	 * On success the retrieved object will be send to this listener
	 * (by calling his function facebookObjectReceived)
	 * On failure the HTTP error code and the id of the object that was
	 * requested is send to the listener.
	 */
	ObjectRequestListener		*mListener;

	/**
	 * mRequester - Pointer to an object that is created each time a request
	 * is made. The type of the object depends on the type of the requested
	 * object.
	 */
...
#endif /* FACEBOOK_OBJECT_MANAGER_H_ */
