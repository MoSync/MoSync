...
namespace Linpack
{
    public class Linpack
    {
        ...
        String PREC = "Double";
        double ZERO = 0.0f; //this must be changed to a double when testing DP performance
        double ONE = 1.0f; //this must be changed to a double when testing DP performance
        double g_startTime;
        String msg = "";
        ...
        public String bench()
        {
            ...
            int arsize;
            long arsize2d, nreps;
            startTime(); //start time measure
            arsize = 200;
            msg = string.Concat(msg, "\n\nLINPACK benchmark, " + PREC + " precision.\n");
            //System.out.print("Machine precision:  %d digits.\n",BASE10DIG);
            msg = string.Concat(msg, "Array size " + arsize + " X " + arsize + "\n");
            msg = string.Concat(msg, "Average rolled and unrolled performance:\n\n");
            msg = string.Concat(msg, "Reps  Time(s)  DGEFA  DGESL  OHEAD  MFLOPS\n");
            msg = string.Concat(msg, "---------------------------------------------------\n");
            ...
            while (true)
            {
                ...
                while (linpack_f(nreps, arsize) < 10.0) //TODO change back to 10 seconds
                    nreps *= 2;
                //free(mempool);
                //System.out.print("\n");
                break; //TODO remove
        ...
        double linpack_f(long nreps, int arsize) {
		    ...
		    if (totalt<0.5f || tdgefa+tdgesl<0.2f)
			    return(0.0f);
		    kflops=2.0f*nreps*ops/(1000.0f*(tdgefa+tdgesl));
		    ...
		    if (toverhead<0.0)
			    toverhead=0.0f;
            //nreps + " " + totalt + " " + 100.0f*tdgefa/totalt + " " + 100.0f*tdgesl/totalt + " " + 100.0f*toverhead/totalt + " " + kflops/1000.0f + "\n"
            msg = string.Concat(msg, string.Format("{0:0000   }  {1:00.0   }  {2:00.0    }  {3:00.0    }  {4:00.0      }  {5:00.0}\n", nreps, totalt, 100.0f * tdgefa / totalt, 100.0f * tdgesl / totalt, 100.0f * toverhead / totalt, kflops / 1000.0f));
	    ...
	    ** ROLLED version
	    */
	    void daxpy_r(int n,double da,double[] dx,int dxpos,int incx,double[] dy,int dypos,int incy) { //TODO dxpos dypos added by me, is the starting index for dx and dy-arrays
		    int i,ix,iy;
	    ...
	    ** UNROLLED version
	    */
	    void daxpy_ur(int n,double da,double[] dx,int dxpos,int incx,double[] dy,int dypos,int incy) {
		    int i,ix,iy,m;
	    ...
	    ** Jack Dongarra, linpack, 3/11/78.
	    */
	    int idamax(int n,double[] dx, int dxpos,int incx) { //TODO The dxpos is added by me and is the starting index of dx[] NEED A DOUBLE version of this
		    double dmax;
		    int i, ix, itemp;
        ...
        ** ROLLED version
        */
        double ddot_r(int n,double[] dx,int dxpos,int incx,double[] dy,int dypos,int incy) {
            double dtemp;
            int i,ix,iy;
        ...
        ** UNROLLED version
        */
        double ddot_ur(int n,double[] dx,int dxpos,int incx,double[] dy,int dypos,int incy) {
            double dtemp;
            int i,ix,iy,m;
	    ...
	    ** UNROLLED version
	    */
	    void dscal_ur(int n,double da,double[] dx,int dxpos,int incx) {
		    int i,m,nincx;
	    ...
	    ** ROLLED version
	    */
	    void dscal_r(int n,double da,double[] dx,int dxpos,int incx) { //TODO do we need dxpos here?
		    int i,nincx;
        ...
        double second() //return the number of seconds since last call to startTime()
        {
