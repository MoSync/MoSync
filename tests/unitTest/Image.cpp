...
void Image::drawImageRegion(int left, int top, Rect *srcRect, Image *img, int transformMode) {
	...
	if (transTopLeftX >= img->width) {BIG_PHAT_ERROR;}
	else if(transTopLeftX < 0) {BIG_PHAT_ERROR;}
	if (transTopLeftY >= img->height) {BIG_PHAT_ERROR;}
	else if(transTopLeftY < 0) {BIG_PHAT_ERROR;}
	if(transBottomRightX < 0) {BIG_PHAT_ERROR;}
	else if(transBottomRightX >= img->width) {BIG_PHAT_ERROR;}
	if(transBottomRightY < 0) {BIG_PHAT_ERROR;}
	else if(transBottomRightY >= img->height) {BIG_PHAT_ERROR;}
