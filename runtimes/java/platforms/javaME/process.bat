...
xgcc -x c -E -D_JavaME -I%JAVASOURCEROOT%/shared -I%JAVASOURCEROOT%/platforms/javaME/src -o build/%2.jtmp %1/%2.jpp 2>&1 | sed "s/\([a-zA-Z/]\+\)\(.[a-zA-Z]\+\):\([0-9]\+\):/\1\2(\3):/"
...
sed "s/^# /\/\//" < build/%2.jtmp > "%TARGETDIR%/src/%2.java"
