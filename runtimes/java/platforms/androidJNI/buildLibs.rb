...
def buildLib(name, buildParams)
	...
	bootmodules = buildParams['bootmodules'] || "wchar,mosync"
	...
	sh("#{ENV['MOSYNCDIR']}/bin/nbuild --platform Android --name #{name} --project #{project} --dst #{ENV['MOSYNCDIR']}/lib --config Debug,Release --lib-variant debug,release --android-ndkbuild-cmd #{$androidNDKPath}/ndk-build --android-version #{$androidVersion} #{srcfiles} --verbose #{libtype} #{includes} #{bootmoduleList} #{moduleList} #{compilerflags} --android-lib-type #{libtype}")
end
...
buildLib("MAStd", { 'src' => "-Skazlib -SMinUI -SMAMath/MAVector3.c -SResCompiler -SMAStd/conprint.c -SMAStd/maassert.c -SMAStd/mastring.c -SMAStd/matime.c -SMAStd/mavsprintf.c -SMAStd/mawvsprintf.c -SMAStd/maxtoa.c" })
buildLib("mautil", { 'src' => "-SHybris -SMAUtil -XMAUtil/DomParser.cpp -XMAUtil/GraphicsOpenGL.c -XMAUtil/XMLDataProvider.cpp -XMAUtil/XPathTokenizer.cpp" })
buildLib("maui", { 'src' => "-SMAUI"})
buildLib("mafs", { 'src' => "-SMAFS/**" })
buildLib("ads", { 'src' => "-SAds" })
buildLib("Facebook", { 'src' => "-SFacebook/**" })
buildLib("nativeui", { 'src' => "-SNativeUI" })
buildLib("Purchase", { 'src' => "-SPurchase" })
buildLib("Wormhole", { 'src' => "-SWormhole/**" })
buildLib("matest", { 'src' => "-SMATest" })
buildLib("mtxml", { 'src' => "-SMTXml" })
buildLib("testify", { 'src' => "-STestify/src" })
buildLib("yajl", { 'src' => "-Syajl/**" })
buildLib("Notification", { 'src' => "-SNotification/**" })
