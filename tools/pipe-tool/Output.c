...
void VerboseError(char *tokenptr)
{
	...
	printf("%03d: ",GetLineNumber(g_FileTop,g_FilePtr));

	for(y=0;y<10;)
	{
	
		if (g_FilePtr == tokenptr)
			printf("<ERROR>");
...
//	.sourcefile 'source\980223.c'
	...
	char *sourcedir = FindDirectiveAbove(g_FilePtr, ".sourcedir");
	char *sourcefile = FindDirectiveAbove(g_FilePtr, ".sourcefile");
	char *sourceline = FindDirectiveAbove(g_FilePtr, ".line");
	char *asmfile = FindDirectiveAbove(g_FilePtr, ".lfile");
	...
	DumpLine = GetLineNumber(g_FileTop, g_FilePtr);
	...
	if (asmfile)
	{
		...
		SkipWhiteSpace();
		GetLFileName();	
		strcpy(AsmFileName, g_Name);
		
		PopTokenPtr();

		AsmLine = GetLineNumber(asmfile, g_FilePtr);
	}
...
void Error(int flag, char *Template, ...)
{
	char 	tbuf[1280];
	va_list	args;
//	char *DebugFilePtr = g_FilePtr;
	
	va_start(args,Template);
	vsprintf(tbuf,Template,args);
	va_end(args);

	if (g_ErrorMode == ErrorMode_Basic)
...
int FileInfo_From_IP(LineInfo *info, int ip)
{
	...
	while(1)
	{
		...
		line = ArrayGet(&g_SLD_Line_Array, ip);
	...
	file = ArrayGet(&g_SLD_File_Array, ip);
