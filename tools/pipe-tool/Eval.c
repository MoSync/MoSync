...
int GetEscCode()
{
	if (octodigit(*g_FilePtr) && *g_FilePtr != '0')
		return GetOctoByte();
...
int GetNum()
{
	...
	if (*g_FilePtr == '0' && (*(g_FilePtr+1) =='x' || *(g_FilePtr+1) == 'X'))
	{
		g_FilePtr+=2;
...
#define TestOp(str,len) if(strncmp(g_FilePtr,str,len) == 0){ ThisOp = str; Oplen = len ;return; }

#define TestOpChar(chr,str) if (*g_FilePtr == chr) { ThisOp = str; Oplen = 1; return;}


#define IsOper(str)		!strcmp(ThisOp,str)

#define SkipOper()		g_FilePtr+=Oplen
...
void Identifier(EVAL *Part1)
{
	while(1)
	{
		...
		if (isdigit(*g_FilePtr) || *g_FilePtr == '\'')
		{	
			Part1->Value	= GetNum();
			Part1->Sym		= NULL;
			Part1->Type		= 0;
...
extern int g_ArgJavaNative;

extern int g_MaxDataIP;
extern int g_ArgRes;
...
void GetIdentifier(EVAL *Part1)
{
	...
	{		
		...
		if (g_ExpIsPure == 0)					// If this is a not pure expression
		{									// Then check code refs
			if (sym->Type == SECT_code)
			{
				...
				g_CodeRef = 1;
				strcpy(g_CodeRefName, ThisLabel);
		...
		if (sym->Type == SECT_bss)
		{
			...
			Part1->Value = sym->Value + g_MaxDataIP;
		}
		...
		if (sym->Type)
		{
			if (g_ExpSection == 0)
			{
				g_ExpSection = sym->Type;
				return;
			}

			if (g_ExpSection == sym->Type)
				return;
