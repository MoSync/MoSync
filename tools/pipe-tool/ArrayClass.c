...
{
    g_CurrentArray = theArray->array;
}

uint arrayGet(int index)
{
    return ArrayGet(g_CurrentArray, index);
}

void arraySet(int index, uint val)
{
    ArraySet(g_CurrentArray, index, val);
}

void arrayAppend(uint val)
{
    ArrayAppend(g_CurrentArray, val);
}
