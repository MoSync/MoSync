...
int SearchDep_Function_Inner(int ip, int ip_end, int depth)
{
	...
	do
	{
		...
		if (ArrayGet(&g_CodeTouchArray, ip) == 0)
		{
			// Say this opcode has been touched

			ArraySet(&g_CodeTouchArray, ip, 1);

			// Check CallArray

			v = ArrayGet(&g_CallArray, ip);
			...
			v = ArrayGet(&g_DataAccessArray, ip);
		...
		switch (thisOpcode.op)
		{
			...
			case _JC_LTU:
			{
				...
				if (ArrayGet(&g_CodeTouchArray, new_ip) == 0)
				{
					// Search the new ip
			...
			case _JPI:
			{
				...
				if (ArrayGet(&g_CodeTouchArray, new_ip) == 1)
					return ip;
			...
			case _CASE:
			{
				...
				for (i=0;i<len+1;i++)
				{
					...
					if (ArrayGet(&g_CodeTouchArray, lab_ip) == 0)
						SearchDep_Function_Inner(lab_ip, ip_end, depth+1);
				}
				
				if (ArrayGet(&g_CodeTouchArray, def_ip) == 1)
					return ip;
...
SYMBOL * FunctionSymFromIP(SYMBOL *localsym)
{
	...
	if (ip > (int) g_CodeLabelArray.hi)
		return 0;

	while(1)
	{
		if (ip < (int) g_CodeLabelArray.lo)
			break;
		
		sym = (SYMBOL *) ArrayGet(&g_CodeLabelArray, ip);
