...
void InitFiles() {
	...
	for (n=g_SLD_File_Array.lo;n<g_SLD_File_Array.hi+1;n++) {
		int file = ArrayGet(&g_SLD_File_Array, n);
		FILE *f;
		const char *fileName;
...
int RebuildCppInst(OpcodeInfo *theOp)
{
	...
	{
		case _PUSH:
			RebuildEmit("	//push %s,%d\n",g_Cpp_reg[theOp->rd], theOp->rs);

			if (REGUSED(g_funcprop.reg_used, REG_sp))
			{
				RebuildEmit("	sp -= %d;\n",theOp->rs*4);
			}
		return 1;
		...
		case _XB:
			RebuildEmit("	%s = (int)((char) %s);", g_Cpp_reg[theOp->rd], g_Cpp_reg[theOp->rs]);		
		break;
...
void CppEmitDivu(OpcodeInfo *theOp, int imm)
{
	if (imm)
	{
		...
		RebuildEmit("	%s = (int) ((unsigned long)(%s)) / ((unsigned long)(%d));", g_Cpp_reg[theOp->rd], g_Cpp_reg[theOp->rd], theOp->imm);
	}
	else
	{
		RebuildEmit("	if(%s == 0) MoSyncDiv0();\n", g_Cpp_reg[theOp->rs]);
		RebuildEmit("	%s = (int) ((unsigned long)(%s)) / ((unsigned long)(%s));", g_Cpp_reg[theOp->rd], g_Cpp_reg[theOp->rd], g_Cpp_reg[theOp->rs]);
	}
}
...
{
	if(unsign)
		RebuildEmit("	if ((unsigned long)%s %s (unsigned long)%s) ",g_Cpp_reg[theOp->rd], str, g_Cpp_reg[theOp->rs]);
	else
...
void RebuildCppProlog(SYMBOL *sym, int isproto)
{
	...
	reg_used = FunctionRegAnalyse(sym, &g_funcprop);
	...
	if (!isproto)
	{
		...
		RebuildEmit("//src_reg    = %s\n", Bin32(g_funcprop.src_reg));
		RebuildEmit("//dst_reg    = %s\n", Bin32(g_funcprop.dst_reg));
		RebuildEmit("//assign_reg = %s\n", Bin32(g_funcprop.assign_reg));
		RebuildEmit("//uninit_reg = %s\n", Bin32(g_funcprop.uninit_reg));
		RebuildEmit("//used_reg   = %s\n", Bin32(g_funcprop.reg_used));
		RebuildEmit("\n");
	}
