...
{
	if(unsign)
		RebuildEmit("	if (((long)(%s) & 0x0ffffffffL) %s ((long)(%s) & 0x0ffffffffL)) ",g_java_reg[theOp->rd], str, g_java_reg[theOp->rs]);
	else
...
//	RebuildJava_Code();
	...
	ArrayWrite(&g_RebuildArray, "StaticCode.java");
...
#if 1
	{
		...
		sprintf(cmd, "gcj -O9 -C %s StaticCode.java", g_GcjFlags);
		printf("%s\n", cmd);
