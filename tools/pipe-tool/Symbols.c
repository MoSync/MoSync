...
#ifdef USE_HASHING
BucketArray g_SymbolHash;
#endif

SYMBOL *g_NextFreeSym;
int g_NextSymbolCount;
...
int InitSymbolTable(void)
{
	...
	g_SymTab = (SYMBOL *) NewPtr(sizeof(SYMBOL) * (SYMMAX + 10));
...
#ifdef USE_HASHING
	BucketArrayInit(&g_SymbolHash, SYMBOL_HASH_SIZE);
#endif

	g_NextFreeSym = g_SymTab;
	g_NextSymbolCount = 0;
...
#ifdef USE_HASHING
	BucketArrayDispose(&g_SymbolHash);
#endif

	DisposeSymbols();

	if (g_SymTab)
		DisposePtr((char *) g_SymTab);
...
{
	SYMBOL *Sym = g_NextFreeSym;

	g_NextFreeSym++;
	g_NextSymbolCount++;

	if (g_NextSymbolCount >= SYMMAX)
...
SYMBOL * FindSymbolsOld(char *string,int sectionStart,int sectionEnd)
{
	...
	while(1)
	{
		elem = BucketArraySearch(&g_SymbolHash, entry, elem, (uint*) &Sym);
...
SYMBOL * StoreSymbol(SYMBOL *NewSym,char *string)
{
	...
	BucketArrayAdd(&g_SymbolHash, entry, (int) Sym);
...
SYMBOL * DefineExtern(char *ThisName)
{
	...
	theSym = FindSymbols(ThisName,section_Extern,section_Extern, g_LocalScope);
...
int EnumIsFunction()
{
	...
	if (g_CurrentFunction)
	{
		...
		if (g_CurrentFunction->Value == lastsymref->Value)
			return 0;
	}
...
int GetEnumValue(char *ThisName)
{
	...
	theSym = FindSymbols(ThisName,section_Enum,section_Enum, g_LocalScope);
...
void DumpEnumEntry(SYMBOL *Sym)
{
	...
	if ((int) Sym >= (int)((int) g_SymTab + sizeof(SYMBOL) * SYMMAX) )
		return;
#endif
...
#define IS_SLASH(c) ((c)=='/' || (c)=='\\')
	...
	if (!g_SLD_Line_Array.array)
		return;

	for (n=g_SLD_Line_Array.lo;n<g_SLD_Line_Array.hi+1;n++)
	{
	
		line = ArrayGet(&g_SLD_Line_Array, n);
		file= ArrayGet(&g_SLD_File_Array, n);
...
void DumpFunctions(FILE *out)
{
	...
	fprintf(out, "CDTOR ");
	fprintf(out, "%s,",   Hex32(g_CDtorStart));
	fprintf(out, "%s\n",  Hex32(g_CDtorEnd));
	return;
}
...
void Dump_MasterIP_Trans(FILE *out)
{
	...
	if (!g_AsmCharArray.array)
		return;

	for (ip=g_AsmCharArray.lo;ip<g_AsmCharArray.hi+1;ip++)
	{
		// n = ip

		ptr = (char *) ArrayGet(&g_AsmCharArray, ip);

		if (ptr)
		{
			if (tptr != ptr)
				line = GetLineNumber(g_FileTop, ptr);
...
void Dump_MasterIP_Trans(FILE *out)
{
	...
	while(1)
	{
		...
		ip = ArrayGet(&g_AsmCharIPArray, offset);
