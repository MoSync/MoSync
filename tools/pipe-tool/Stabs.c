...
int Stabs_Init()
{
	...
	ArrayInit(&g_StabsArray, sizeof(char), 0);
	Stabs_Reset();
	return 1;
}
...
void Stabs_Write()
{
	ArrayWrite(&g_StabsArray, g_StabsName);
}
...
void StabsEmit(char *Template, ...)
{
	...
	for (n=0;n<len;n++)
		ArrayAppend(&g_StabsArray, Str[n]);
