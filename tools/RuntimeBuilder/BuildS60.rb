...
class RuntimeBuilder
	...
	def build_symbian(runtime_dir, mode, version)
		...
		if version == "s60v5"
			comPath = $SETTINGS[:s60v5_compiler]
			if(!File.exist?(comPath + 'arm-none-symbianelf-gcc.exe'))
				raise "FATAL ERROR: Cannot find 5th edition CSL Arm Toolchain at '#{comPath}'"
			end
			cmd = "#{cwd}/build_s60v5.bat \"#{comPath}\""
			sh(cmd)
		else
