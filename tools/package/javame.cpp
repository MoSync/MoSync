...
static void writePermissions(ostream& stream, const SETTINGS& s, const RuntimeInfo& ri) {
	...
	if (outPermissions.size() > 0) {
		write72line(stream, string("MIDlet-Permissions: ") +
			delim(outPermissions, permissionDelim));
		stream << "\n";
	}
	if (outOptPermissions.size() > 0) {
		write72line(stream, string("MIDlet-Permissions-Opt: ") +
			delim(outOptPermissions, permissionDelim));
		stream << "\n";
	}
}
