...
// Parses the language definition line
	{
	Verbage(_T("processing languages"));
...
// Parses the pkg header line
	...
	if(!m_pSISWriter->GetNoLanguages())
		{
		Verbage(_T("no languages defined, assuming English only"));
		LANGNODE *pNode = new LANGNODE;
		if (!pNode) throw ErrNotEnoughMemory;
...
// Parses a package (component SIS file) line
	{
	Verbage(_T("processing embedded package file"));
...
// Parses a dependency line                         
	{
	Verbage(_T("processing dependency"));
...
void CParsePkg::ParseSignatureL()
{
	...
	Verbage(_T("processing signature"));
...
const _TCHAR* CParsePkg::GetTokenText(TOKEN aToken)
	...
	switch(aToken)
		{
		case NUMERIC_TOKEN:
			return _T("numeric value");
		case ALPHA_TOKEN:
			return _T("alphanumeric value");
		case QUOTED_STRING_TOKEN:
			return _T("quoted string");
		case AND_TOKEN:
			return _T("AND");
		case OR_TOKEN:
			return _T("OR");
...
//			  pdwNumber  - The output number
	...
	while(parse) {
		...
		if(iswxdigit(tmp_char) || tmp_char == 'x') {
...
// Returns  : Yes or No
	...
	*pdwSize = 0;
	...
	try
		...
		m_pObserver->DoVerbage(_T("warning :file does not exist"));
		fFound=TRUE;
		}
	return fFound;
	}
