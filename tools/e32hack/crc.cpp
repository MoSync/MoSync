...
//            wLength - The length of the memory block
{
	...
	uint16_t crc = wCheck;
	while (pB < pE)
		crc = (uint16_t)((crc << 8) ^ crcTab[((crc >> 8) ^ *pB++) & 0xff]);
	return crc;
}
...
// Inputs   : pPtr    - Pointer to a block of memory
{
	...
	const unsigned char *pB = (const unsigned char *)pPtr;
	const unsigned char *pE = pB + (KUidInstallNumUid * sizeof(uint32_t));
	unsigned char buf[(KUidInstallNumUid * sizeof(uint32_t))>>1];
	...
	return Crc(&buf[0], (KUidInstallNumUid * sizeof(uint32_t))>>1, 0);
}

void calcUidCrc(void* buffer) {
	uint* uid = (uint*)buffer;
	uint32_t dwCheck = 0;
	uint16_t wCheck = UIDCheck(((char *)&uid[0]) + 1);
	dwCheck = (uint32_t)(wCheck) << 16;
	wCheck = UIDCheck((char *)&uid[0]);
	dwCheck |= wCheck;
	uid[3] = dwCheck;
}
//UID CRC code ends here
