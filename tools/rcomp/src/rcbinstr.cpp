...
void ResourceDataStream::StartOfBlockWithSizePrefix(DataType aDataTypeOfSizePrefix)
	...
	iArrayOfMarks.AppendMark(iNumberOfBytesUsed, EMarkType_StartOfBlockWithSizePrefix, (size_t)sizeOfBlockWhenUncompressed);
	}
...
void ResourceDataStream::StartOfCompressedUnicodeRun(int aUncompressedUnicodeSizeInBytes, const unsigned char* aUncompressedUnicodeBuffer)
	...
	if (!iContainsCompressedUnicode)
		...
		if (insertZeroLengthCompressedUnicodeRunAtStart)
			{
			iArrayOfMarks.InsertMark(0, 0, EMarkType_StartOfCompressedUnicodeRun, (size_t)BinaryBuffer::New(0, NULL)); // mark the insertion point for the initial zero-length compressed-Unicode run
	...
	iArrayOfMarks.AppendMark(iNumberOfBytesUsed, EMarkType_StartOfCompressedUnicodeRun, (size_t)BinaryBuffer::New(aUncompressedUnicodeSizeInBytes, aUncompressedUnicodeBuffer));
...
void ResourceDataStream::EnquireStreamPositionWhenKnown(unsigned long& aStreamPosition)
	{
	iArrayOfMarks.AppendMark(iNumberOfBytesUsed, EMarkType_EnquireStreamPositionWhenKnown, (size_t)&aStreamPosition);
	}
...
startOfFirstPass:
	...
	const size_t* lengthOfLastRun=NULL;
	for (int pass=0; pass<3; ++pass)
		...
		size_t* addressToWriteSizeOfCompressedIntegerTo=NULL; // used on the first pass
		...
		for (int i=0; i<numberOfMarks; ++i) // must iterate forwards so that lengthOfLastRun is correctly assigned (after the end of this loop) to point to the iOtherData (i.e. run-length) of the last EMarkType_EndOfCompressedUnicodeRun mark
			...
			case EMarkType_StartOfBlockWithSizePrefix:
				...
				if (pass==0)
					...
					mark.iOtherData=(size_t)(NumericValue*)NULL;
					}
				}
				break;
			...
			case EMarkType_StartOfCompressedUnicodeRun:
				...
				if (pass==0)
					...
					{
					const size_t* const lengthOfOtherStuffRun=&nextMark.iOtherData;
					...
					mark.iOtherData=(size_t)(BinaryBuffer*)NULL;
...
void ResourceDataStream::ConvertCompressedRunToUncompressed(int aMarkIndexOfStartOfCompressedUnicodeRun)
	...
	markForStartOfCompressedUnicodeRun->iOtherData=(size_t)(BinaryBuffer*)NULL;
